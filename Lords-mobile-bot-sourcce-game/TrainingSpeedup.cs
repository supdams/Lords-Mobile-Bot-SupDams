...
public class TrainingSpeedup : SpeedupBase
{
  public TrainingSpeedup(EQueueBarIndex QueueBar)
  {
    ...
    this.FilterType2 = (byte) 21;
  }

  public override void SendImmediate() => DataManager.Instance.SendFinishtraining();
  ...
  public override void CustomSort(List<ushort> Data, int BagCount)
  {
    ...
    for (int index = 0; index < BagCount; ++index)
    {
      ...
      else if ((byte) instance.EquipTable.GetRecordByKey(this.CustomList[index]).PropertiesInfo[0].Propertieskey == (byte) 21)
      {
        Data.Add(this.CustomList[index]);
        this.CustomList.RemoveAt(index);
        --BagCount;
